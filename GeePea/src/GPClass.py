...
#     logP = -0.5 * np.dot(r.T,LA.cho_solve(self.choFactor[useK],r,check_finite=False)) - 0.5 * self.logdetK[useK] - (r.size/2.) * np.log(2*np.pi)
    ri = LA.cho_solve(self.choFactor[useK], r, check_finite=False)  # decorrelate the residuals
    for i in range(519):
      rsi[i] = ri[i] / (wi[i])  # Weight the residuals
    for i in range(519):
      if np.abs(rsi[i]) < b:
        L1[i] = -wi[i] ** 2 * (np.square(rsi[i]) / 2)  # -np.log(1/np.sqrt(2*np.pi)))
      else:
        L1[i] = -wi[i] ** 2 * (b * (np.abs(rsi[i])) - 0.5 * b ** 2)  # -np.log(1/np.sqrt(2*np.pi)))
    L = np.sum(L1)
    return np.float(L)
...
#     return K
  ...
  def Prstatistics(self):
    ...
    for kk in range(m):
        ...
        for ii in range(m):
            z[ii,:]=np.dot(np.transpose(H[ii,:]),v[kk,:])
